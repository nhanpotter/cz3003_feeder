...
func init(params):
	...
	print("test scene called with params:" + str(params))

func _on_Button_pressed():
	var next_scene = Scene_Manager.get_scene_path("test2")
	print(next_scene)
...
func _on_Button3_pressed():
	if Network_Services.is_logged_in():
		...
		Network_Services.get_user_info(self,"handle_data")
		Network_Services.get_avatar(self,"handle_data2")
	else:
		print("not logged in")
...
func handle_data2(result, response_code, headers, body):
	var json = JSON.parse(body.get_string_from_utf8())
