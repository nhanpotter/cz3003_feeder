...
onready var stage7 = $Stage7
onready var stage6 = $Stage6
...
func _initialize_stages(worlds):
	var stages = [stage1,stage2,stage3,stage4,stage5,stage6,stage7]
	var count = 0
	for world in worlds:
		if count <6:
...
func _on_Button6_pressed():
	if _check_level($Stage6) == false:
		_display_level_lock()
	else:
		print("entering world..") #debug
		World_Manager.init_world(worlds[5])
	pass

func _on_Button7_pressed():
	if _check_level($Stage7) == false:
		_display_level_lock()
	else:
		print("entering world..") #debug
		World_Manager.init_world(worlds[5])
